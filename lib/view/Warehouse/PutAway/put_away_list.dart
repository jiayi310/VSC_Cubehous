...
class PutAwayListPage extends StatefulWidget {
  const PutAwayListPage({super.key});

  @override
  State<PutAwayListPage> createState() => _PutAwayListPageState();
}

class _PutAwayListPageState extends State<PutAwayListPage> {
  // Session
  String _apiKey = '';
  String _companyGUID = '';
  int _userID = 0;
  ...
  Future<bool> _deletePutAway(int docID) async {
    try {
  ...
  Future<void> _fetch({required int page}) async {
    ...
    try {
      ...
      final result = PutAwayResponse.fromJson(response as Map<String, dynamic>);
  ...
  Future<void> _onDeleteTap(PutAwayListItem item) async {
    if (!_hasAccess('PUTAWAY_DELETE')) {
      _showNoAccessDialog();
      return;
    }
    final confirmed = await _confirmDelete(item.docNo);
    if (confirmed != true) return;
    final ok = await _deletePutAway(item.docID);
    if (ok && mounted) {
      setState(() => _items.removeWhere((e) => e.docID == item.docID));
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(const SnackBar(
          content: Text('Put Away deleted'),
          behavior: SnackBarBehavior.floating,
        ));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Put Away', style: TextStyle(fontWeight: FontWeight.w600)),
        centerTitle: true,
                    ...
                    Material(
                      ...
                      child: InkWell(
                        ...
                        child: const SizedBox(
                            width: 44, height: 44, child: Icon(Icons.search, size: 20)),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Stack(
              ...
              child: _PutAwayTile(
                ...
                onTap: () async {
                  final result = await Navigator.push<bool>(
                    context,
                    MaterialPageRoute(
                      builder: (_) => PutAwayDetailPage(docID: item.docID),
                    ),
                  );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Failed to load put away records',
...
class _PutAwayTile extends StatelessWidget {
  final PutAwayListItem item;
  final DateFormat dateFmt;
  final VoidCallback onTap;

  const _PutAwayTile({required this.item, required this.dateFmt, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    try {
      createdDate = DateTime.parse(item.createdDateTime);
    } catch (_) {}
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Icon(Icons.inventory_2_outlined, size: 22, color: primary),
          ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const Spacer(),
                    Text(
                      createdDate != null ? dateFmt.format(createdDate) : '',
                ...
                // Row 2: stockCode + uom
                Text(
                  '${item.stockCode}  ·  ${item.uom}',
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
                const SizedBox(height: 4),
                // Row 3: location › storage
                Row(
                  children: [
                    Icon(Icons.location_on_outlined, size: 12, color: muted),
                    const SizedBox(width: 2),
                    Text(
                      item.storageCode.isNotEmpty
                          ? '${item.location} › ${item.storageCode}'
                          : item.location,
                      style: TextStyle(fontSize: 11, color: muted),
                    ),
                  ],
