...
// ─────────────────────────────────────────────────────────────────────
// Multi Customer Picker Page
// ─────────────────────────────────────────────────────────────────────

class MultiCustomerPickerPage extends StatefulWidget {
  final String apiKey;
  final String companyGUID;
  final int userID;
  final String userSessionID;
  final List<Customer> initialSelected;

  const MultiCustomerPickerPage({
    super.key,
  ...
  @override
  State<MultiCustomerPickerPage> createState() =>
      _MultiCustomerPickerPageState();
}

class _MultiCustomerPickerPageState extends State<MultiCustomerPickerPage> {
  final _searchCtrl = TextEditingController();
  final _scrollCtrl = ScrollController();

  List<Customer> _customers = [];
  late Set<int> _selectedIDs;
  late Map<int, Customer> _selectedMap;
  ...
  void initState() {
    super.initState();
    _selectedIDs =
        widget.initialSelected.map((c) => c.customerID).toSet();
    _selectedMap = {
      for (final c in widget.initialSelected) c.customerID: c
    };
    _fetch(page: 0);
  }
  ...
  Future<void> _fetch({required int page}) async {
    ...
    String nCustomerType = "";
    String nSalesAgent = "";
    String nPriceCategory = "";

    if (_filterCustomerTypeIDs.isNotEmpty) {
      nCustomerType = _filterCustomerTypeIDs.join(",");
    }

    if (_filterSalesAgentIDs.isNotEmpty) {
      nSalesAgent = _filterSalesAgentIDs.join(",");
    }

    if (_filterPriceCategoryList.isNotEmpty) {
      nPriceCategory = _filterPriceCategoryList.join(",");
    }
    ...
    try {
      final response = await BaseClient.post(
        ...
        body: {
          ...
          'filterCustomerTypeIdList': nCustomerType.isEmpty
              ? null
              : nCustomerType,
          'filterSalesAgentIdList': nSalesAgent.isEmpty
              ? null
              : nSalesAgent,
          'filterPriceCategoryList': nPriceCategory.isEmpty
              ? null
              : nPriceCategory,
        },
      );
      ...
      final data = (raw['data'] as List<dynamic>?)
              ?.map((e) =>
                  Customer.fromJson(e as Map<String, dynamic>))
              .toList() ??
          [];
      final total =
      ...
      if (mounted) {
        setState(() {
          ...
          _totalCount = total;
          _totalPages =
              _pageSize > 0 ? (total / _pageSize).ceil() : 1;
  ...
  void _toggle(Customer c) {
    setState(() {
      if (_selectedIDs.contains(c.customerID)) {
        _selectedIDs.remove(c.customerID);
        _selectedMap.remove(c.customerID);
      } else {
        _selectedIDs.add(c.customerID);
        _selectedMap[c.customerID] = c;
      }
    });
  }
  ...
  void _showFilterSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => _MultiCustFilterSheet(
        sortBy: _sortBy,
        sortAsc: _sortAsc,
                      ...
                      children: [
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (ctx, i) {
                              ...
                              final selected =
                                  _selectedIDs.contains(c.customerID);
                              return ListTile(
                        ...
                        Padding(
                          ...
                          child: SizedBox(
                            ...
                            child: FilledButton(
                              onPressed:
                                  _selectedIDs.isNotEmpty ? _confirm : null,
...
class _MultiCustPaginationBar extends StatelessWidget {
...
class _MultiCustFilterSheet extends StatefulWidget {
  final String sortBy;
  final bool sortAsc;
  ...
  @override
  State<_MultiCustFilterSheet> createState() =>
      _MultiCustFilterSheetState();
}

class _MultiCustFilterSheetState extends State<_MultiCustFilterSheet> {
  late String _sortBy;
  late bool _sortAsc;
  ...
  void initState() {
    ...
    _selectedCustomerTypeIDs =
        Set.from(widget.selectedCustomerTypeIDs);
    _selectedSalesAgentIDs = Set.from(widget.selectedSalesAgentIDs);
    _selectedPriceCategoryList =
        Set.from(widget.selectedPriceCategoryList);
    _loadOptions();
  }
    ...
    try {
      ...
      setState(() {
        _customerTypes = (results[0] as List<dynamic>)
            .map((e) =>
                CustomerType.fromJson(e as Map<String, dynamic>))
            .toList();
        _salesAgents = (results[1] as List<dynamic>)
            .map((e) =>
                SalesAgent.fromJson(e as Map<String, dynamic>))
            .toList();
        _isLoading = false;
      });
    } catch (_) {
